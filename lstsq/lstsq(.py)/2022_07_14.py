...
class NesterovMethod(Optimizer):
    ...
    def kernel_function(self):
        ...
        """
        #define x (threadIdx.x)
        __global__ void nesterov_method (float* velocity, float* theta, float* grad, float learning_rate, float rho) {
            
            theta[x] += rho * velocity[x] - learning_rate * grad[x];
        ...
        __global__ void nesterov (float* theta, float* velocity, float rho) {
            
            theta[x] += rho * velocity[x];
...
class HyperParameterTuning:
    def __init__(self, A, b, optimizer=None):

        self.A = A
        self.b = b

        self.lr_upper  = 1 / A
        self.lr_downer = 1 / (A * 100)
    ...
    def tune(self):
        if self.optimizer == None:
            key = f"learning_rate: {self.learning_rate}"

        else:
            key = f"learning_rate: {self.learning_rate}, beta: {self.beta}"
